        ...
        <template slot-scope="scope">
          <el-button
            v-if="
              ...
              parseFloat(scope.row.totalAmount) > 0
            "
            size="mini"
            type="text"
            icon="el-icon-money"
    ...
    >
      <el-form label-width="80px">
        <el-form-item label="商户号">
          <div>{{ payRows.mchOrderNo }}</div>
        </el-form-item>
        <el-form-item label="商品名">
          <div>{{ payRows.subject }}</div>
        </el-form-item>
        <el-form-item label="商品描述">
          <div>{{ payRows.body }}</div>
        </el-form-item>
        <el-form-item label="支付金额">
          <div>{{ payRows.amount }} {{ payRows.currency }}</div>
        </el-form-item>
  ...
  methods: {
    ...
    handlePay(row) {
      precreatePayorder({ financeId: row.id }).then((response) => {
        ...
        this.openPay = true;
        this.deadline2 = Date.now() + 5 * 60 * 1000;
      });
    },
    hilarity() {
      this.$notify({
